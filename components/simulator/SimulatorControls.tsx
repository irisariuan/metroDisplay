...
export function SimulatorControls({
	...
	const setLangMode = (
		value: React.SetStateAction<SimulatorControlState["langMode"]>,
	) => set("langMode", value);
		...
		<div className="mt-5.5 flex flex-col gap-4 rounded-3xl border-3 border-ink bg-paper p-4.5 shadow-hard-s">
			...
			<div className="grid grid-cols-2 items-stretch gap-3">
					...
					<div className="grid grid-cols-3 gap-4">
						...
						<div className="flex flex-col gap-3">
							<div>
								...
								<input
									...
									onChange={(ev) =>
										setDoorNoticeMs(Number(ev.target.value))
									}
									style={
										{
						...
						<div>
							...
							<input
								...
								onChange={(ev) =>
									setStationStayMs(Number(ev.target.value))
								}
								style={
									{
						...
						>
							...
							>
								...
								<p className="mt-1.75 font-body text-sm leading-[1.3] text-muted">
									Ads and Metro notices hold the ticker until
									this share of the current leg is complete.
								</p>
							</div>
						</div>
						<div
						...
						>
							<div className="mb-1.75 flex items-center justify-between gap-2.5">
								<span className="font-mono text-sm font-bold tracking-widest text-ink">
									CONTENT DISPLAYABLE PLAYLIST · ENGLISH +
									OPTIONAL JAPANESE
								</span>
								<button
									className="lc-btn"
									onClick={() =>
										dispatch({ type: "addMarqueeItem" })
									}
									style={{
										padding: "5px 9px",
										background: "var(--acid)",
							...
							<div className="flex max-h-65 flex-col gap-1.5 overflow-y-auto pr-0.75">
								{announcements.map((item, index) => (
									<div
										...
										style={{
											gridTemplateColumns:
												"auto auto 70px minmax(150px, 1fr) minmax(150px, 1fr) minmax(150px, auto) auto",
											boxShadow: "2px 2px 0 var(--ink)",
										}}
									>
										<Switch
										...
										<select
											...
											onChange={(ev) =>
												dispatch({
													...
													field: "type",
													value: ev.target
														.value as AnnouncementContentType,
												})
											}
				...
				>
					<div className="mb-2 flex items-center justify-between gap-3">
						<div>
							<div className="font-display text-[25px] leading-none">
								AUDIO ANNOUNCEMENT
							</div>
							<div className="mt-1 font-mono text-[10px] font-bold tracking-widest text-muted">
								COMPOSED FROM FRAMEWORK + STATION + TRANSFER
								CLIPS
							</div>
						</div>
						<Switch
					...
					<div className="grid gap-2.5 lg:grid-cols-2">
						<div className="rounded-md border-2 border-ink bg-paper-2 p-2">
							<div className="mb-1.5 font-mono text-[10px] font-bold tracking-widest">
								CURRENT STATION · {currentStation.ja} /{" "}
								{currentStation.en}
							</div>
							<div className="grid grid-cols-2 gap-1.5">
								{(["ja", "en"] as const).map((audioLang) => {
									const key = stationAudioKey(
										currentStation,
										audioLang,
									);
									return (
										<AudioClipControl
											...
											audioKey={key}
											overridden={Boolean(
												announcementAudioOverrides[key],
											)}
					...
					<label className="mt-2.5 flex items-center gap-3 font-mono text-[10px] font-bold tracking-widest">
						...
						<input
							...
							onChange={(event) =>
								setDepartureMajorStationCount(
									Number(event.target.value),
								)
							}
							style={
								{
									"--range-fill": `${departureMajorStationCount * 20}%`,
								} as React.CSSProperties
							}
						/>
					...
					<label className="mt-2.5 flex items-center gap-3 font-mono text-[10px] font-bold tracking-widest">
						...
						<input
							...
							style={
								{
									"--range-fill": `${announcementVolume * 100}%`,
								} as React.CSSProperties
							}
						/>
