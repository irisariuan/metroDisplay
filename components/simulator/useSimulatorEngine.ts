"use client";
/* Simulator engine: owns all state, effects, journey loop, and audio wiring.
 *
 * Extracted from MetroSimulator so the same engine can back three surfaces:
 *  - the combined page (`/`), which renders display + controls locally,
 *  - the `/display` device, which runs this engine authoritatively, and
 *  - (indirectly) the `/control` device, which mirrors the snapshot this hook
 *    exposes rather than running the engine itself.
 * The returned shape mirrors exactly the props MetroSimulator used to build
 * inline, so consumers stay thin. */
...
export function useSimulatorEngine() {
	...
	const displayProps = {
		...
		tickerColor:
			nextMarqueeMessageVisible &&
			announcementContents.currentAudioType !== "departure"
				? annColor
				: "var(--paper)",
	...
	const context = {
		...
		stopAnnouncementAudio: announcementContents.stopAnnouncementAudio,
		isAudioClipAvailable: announcementContents.isAudioClipAvailable,
	...
	return {
		...
		displayProps,
		context,
		audioRef: announcementContents.audioRef,
		audioProps: announcementContents.audioProps,
		status: { phase: journey.phase, auto },
		// Extra fields the split-mode /control device needs mirrored but that
		// the local combined page ignores.
		manifestClipKeys: announcementContents.manifestClipKeys,
	};
}
